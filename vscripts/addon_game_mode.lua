...
function COverthrowGameMode:OnThink()
	...
	for i=1, #all do
        ...
        --    self:CreateMythicWeapon(hero, "immo6generate", false, 0, 0 ,0, true)
        --end
        --QUESTS
        --final quest
...
function COverthrowGameMode:SaveCharSendToServer(data, hero, steamid)
    ...
    if IsServer() then
        --print(" try send 2")
        local request = CreateHTTPRequestScriptVM( "POST", "http://catze.eu/savechar_v18_season_5.php" )
		...
		if source:IsHero() then
            if source.super_aggro_tank and not event.ignore_super_aggro_tank then
                ...
                if level_tank_ability and level_tank_ability >= 4 then
                    local aggro_bonus_armor = 1 + source:GetPhysicalArmorValue() * 0.05
                    if aggro_bonus_armor < 1 then
                        aggro_bonus_armor = 1
                    end
                    local aggro_bonus_resist = 1 + source:GetMagicalArmorValue() * 100 * 0.1
...
if heroName == "npc_dota_hero_dazzle" then
    ...
    if ability1 ~= nil then
        ...
        local stunTime = 20 --COverthrowGameMode.time_to_choose_ml
	...
	if killedUnit:IsRealHero() then
		...
		if self.junglemode then
			if killedUnit.pvelives and killedUnit.pvelives > 0 then
				...
				if ((killedUnit:HasModifier("modifier_item_crit_pure_immortal3") or killedUnit:HasModifier("modifier_item_crit_myth")) and math.random(1,100) <= 20) or (killedUnit.talents and killedUnit.talents[63] and killedUnit.talents[63] > 0 and math.random(1,100) <= chance_not_lose_life) then
                    --print("no life lost")
                ...
                if killedUnit.combat_system_ability then
                    Timers:CreateTimer(2.1,function() 
                        killedUnit.combat_system_ability:ApplyDataDrivenModifier(killedUnit, killedUnit, "modifier_talent_spiritwolf", {duration = 10})
                    end)
                end
...
function COverthrowGameMode:FilterDamage( filterTable )
  ...
  --block agi moonglaive shield 45%
    ...
    if victim.talents and victim.talents[95] and victim.talents[95] > 0 then
        local baseStats = GetIntellectCustom(victim)
        if victim:HasModifier("modifier_pathbuff_095") then
            baseStats = baseStats + GetStrengthCustom(victim) * 0.5
        end
        block = baseStats * 0.1 * victim.talents[95]
        newdamage = newdamage - block
        if newdamage < 0 then
            newdamage = 0
        end
    ...
    --wolf rider
    if victim.talents and victim.talents[60] and victim.talents[60] > 0 and victim:IsMoving() then
        local wolf_dmg_factor = (1 - 0.1 * victim.talents[60])
	...
	if victim.talents and victim.combat_system_ability then
        ...
        if victim.talents[57] and victim.talents[57] > 0 and not victim:HasModifier("modifier_talent57cd") and lifeafterdamage < victim:GetMaxHealth() / 2 then
            newdamage = 0
...
function COverthrowGameMode:SaveChar(hero)
  ...
  local steamid = tostring(PlayerResource:GetSteamAccountID(playerid))
		...
		if not (PlayerResource:GetConnectionState(playerid) == DOTA_CONNECTION_STATE_DISCONNECTED or PlayerResource:GetConnectionState(playerid) == DOTA_CONNECTION_STATE_ABANDONED) then
            ...
            local aggro_amount = GetDedicatedServerKey("GetAggroKey")
            --print(aggro_amount)
...
function LoadMyChar(caster)
    if IsServer() then
        ...
        --new sys
        local request = CreateHTTPRequestScriptVM( "POST", "http://catze.eu/loadchar_v13_season_5.php" )
...
function TempleDifficultyChange(event, args)
    ...
    if max_ml_by_kill_score < COverthrowGameMode.jungledifficulty + step*value then
        local required_boss_score = (COverthrowGameMode.jungledifficulty + step*value) * 20
...
function GetHighestAllowedMonsterLevel()
    ...
    local allowed_ml = math.floor(0.1 + highest_kill_score / 20)
       ...
       for j=0, #data-2 do
          if data[j] and steamid > 0 then
            ...
            if id and steamid == id and not hero.auto_loaded then
                --print("j value " .. j)
                --print(data[0])
                local goldindex = j+63
                local bankindex = goldindex-1
                local temple_abil_index = goldindex + 12
				...
				if temple_version == 2 and bank_slots > 0 then
                    ...
                    if bank_slots >= 6 then
                        ...
                        for n=j+47, j+58 do
                            if data[n] and data[n] ~= "no" then
                                local stash_index = math.floor(1 + (n-47) / 3) + 1
                                local stash_slot = ((n-47) % 3) + 1
                        ...
                        for n=j+64, j+75 do
                            if data[n] and data[n] ~= "no" then
                                local stash_index = math.floor(1 + (n-64) / 3) + 1
                                local stash_slot = ((n-64) % 3) + 4
                                local newitem = "item_" .. data[n]
					...
					if hero.bought_stash_count > 0 then
                        ...
                        for s2=stash_2_index, stash_2_index + 6 do
                            ...
                            Timers:CreateTimer(0.25,function() --wait a moment for path talent rings to update
                                --if true then
                                    local pathpoints = data[81]  --with 7 artifact slots 49+4
                                    --print(pathpoints)
...
function COverthrowGameMode:PopulateTemple(zone)
	...
	if zone == 1 then
        ...
        --unit:AddAbility("pve_wave_caller_2"):SetLevel(1)
    ...
    for i=1, horde_factor do
        ...
        unit:AddAbility("pve_affix_jumpstomp_2"):SetLevel(1) --pve_affix_shadow_claw
        unit:SetRenderColor(255, 255, 255)
        ...
        unit:AddAbility("pve_affix_jumpstomp_2"):SetLevel(1)
        unit:SetRenderColor(255, 255, 255)
        self:SetTempleStats(unit,scale,50*act_1_dmg_factor,750,20,2*affixinact1)
        unit:SetForwardVector(Vector(0,1,0))
        ...
        unit:AddAbility("pve_affix_pure_dot"):SetLevel(1)
		...
		--boss 4
		unit = CreateUnitByName("temple_wolf_boss_4", c3+Vector(1600,-1200,0), true, nil, nil, DOTA_TEAM_BADGUYS )
		unit.act = zone
    ...
    for i=1, horde_factor do
        ...
        --traps
        unit = CreateUnitByName("temple_spike_trap", c1+Vector(1250+2200,2900+50,0), true, nil, nil, DOTA_TEAM_BADGUYS )
        unit.exp_factor = 5
        self:SetTempleStats(unit,scale,750,100,0,0)
        ...
        unit:SetForwardVector(Vector(0,-1,0))
		--boss 2
		unit = CreateUnitByName("temple_sand_boss_2", b2, true, nil, nil, DOTA_TEAM_BADGUYS )
		unit.act = zone
    ...
    for i=1, horde_factor do
        ...
        --unit = CreateUnitByName("temple_missile_trap", c3+Vector(650,-1750,0), true, nil, nil, DOTA_TEAM_BADGUYS )
        --unit.exp_factor = 5
        --unit.chance_for_champion = 10
        --self:SetTempleStats(unit,scale,750,100,20,0)
        --unit:SetForwardVector(Vector(0,1,0))
	...
	for i=1, 62 do
     ...
     Timers:CreateTimer(i*0.75+extra,function() 
			...
			if i >= 58 then
				unit.canleavespawnpos = true
				if roll == 3 then
					COverthrowGameMode:SetTempleStats(unit,scale,100,6000,100,2)
					unit:SetModelScale(1.3)
                  elseif roll == 2 then
                     COverthrowGameMode:SetTempleStats(unit,scale,150,4000,100,2)
                     unit:SetModelScale(1.3)
                 else
                     COverthrowGameMode:SetTempleStats(unit,scale,125,5000,100,2)
                     unit:SetModelScale(1.1)
                 end
                 unit.act = 6
                 unit.templeboss_mount = 1
...
function COverthrowGameMode:SetTempleStats(target, scale, dmg, hp, reward, affixes)
    ...
    --hp = hp * 5 * interpolation_factor + hp * 1.5 * (1 - interpolation_factor)
    ...
    dmg = dmg * playerscale
    dmg = dmg * 1.5 * interpolation_factor + dmg * (1 - interpolation_factor)  --more dmg since season 5: 2 - 1.1
    ...
    if abil then
        abil:SetLevel(1)
        if isboss and not target.isgoblin then
...
function Act13Event(event)
    ...
    for i=1, 100 do
        local delay = 0
        if i >= 42 then --events 20 40 70 100
            delay = delay + 25
        end
        if i >= 73 then
            delay = delay + 20
        ...
        Timers:CreateTimer(i*4.0+delay,function()
            ...
            for j=1,max do
                local unit = CreateUnitByName(unitname, spawnpos, true, nil, nil, DOTA_TEAM_BADGUYS )
                if i % 10 == 0 and (j == 1 or i == 20) and i ~= 40 and i ~= 41 and i ~= 70 and i ~= 71 and i ~= 72 and i ~= 100 then --every 10th a champ, but not when boss spawns
                    unit.chance_for_champion = 100
                    unit.act = 13
                end
                local loot = 0
